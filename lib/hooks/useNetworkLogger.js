...
import { subscribeToNetworkLogs, getNetworkLogs, clearNetworkLogs, repeatNetworkRequest, } from '../utils';
export const useNetworkLogger = () => {
    ...
    const filteredLogs = useMemo(() => {
        return logs.filter(log => {
            ...
            if (statusFilter.length > 0) {
                ...
                if (statusFilter.includes('success') && !isSuccess)
                    return false;
                if (statusFilter.includes('error') && !isError)
                    return false;
                if (statusFilter.includes('pending') && !isPending)
                    return false;
            }
            return true;
        });
    ...
    const networkStats = useMemo(() => {
        if (logs.length === 0)
            return null;
        const completedRequests = logs.filter(log => log.status >= 200 && log.status < 300).length;
        ...
        const averageResponseTime = totalResponseTime > 0
            ? Math.round(totalResponseTime / logs.filter(log => log.duration > 0).length)
            : 0;
