...
export function setupNetworkLogging(options: SetupNetworkLoggingOptions = {}) {
  const {
    clients = ['axios', 'fetch'],
    axios: axiosInstance,
...
export function autoSetupNetworkLogging(
  config?: SetupNetworkLoggingOptions['config'],
) {
  const clients: Array<'axios' | 'fetch' | 'react-query'> = [];
